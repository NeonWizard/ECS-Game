		...
		for (i=0; i<entityIdsCollidedWith.length; i++) {
			...
			if (Object.keys(ECS.entities).length < 30) {
				for (var k=0; k < numNewEntities; k++) {
					...
					if (Math.random() < 0.8) {
						newEntity = new ECS.Entity();
						newEntity.addComponent( new ECS.Components.Appearance() );
						newEntity.addComponent( new ECS.Components.Position() );
						newEntity.addComponent( new ECS.Components.Collision() );
